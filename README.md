## file-rehasher 文件洗码工具

file-rehasher是修改文件哈希值（MD5 / SHA-1 / SHA-256 / SHA-512）的文件洗码工具，有网页版和带有GUI的桌面python版

我们希望它可以在以下场景帮助到你：

1.保护你的文件不被轻易识别检测。**在本地或网盘存储文件时，降低被自动识别、标记或失效的可能性，让文件更稳定、更不容易“和谐”。** 同时在一定程度上规避监控，尤其是监控你在本地存储了哪些被标记的敏感文件。
...
本工具仅供学习与个人使用。本工具只能规避通过检测追踪文件哈希的监控，增加监控追踪成本。不能抵御AI文件内容识别、图片视频隐藏水印、人工审查等监控手段。使用前请自行备份原始文件，作者不对数据损失承担责任。请勿将本工具用于规避版权保护。
